...
import random
import warnings
from model_feature import MLModel
...
class Net(pl.LightningModule):
    ...
    def collate_fn_text(self, batch):
        ...
        inputs = {'persons': [], 'posts': [], 'responses': [], 'p_r': [], 'his': [],'features':[]}
        for line in batch:
    ...
    def test_step(self, batch, batch_nb):
        return self.forward(batch, is_train=False)

    def test_epoch_end(self, outputs) -> None:

        torch.save(outputs,'test_sample.pt')
...
if __name__ == "__main__":
    ...
    output_path = config.output_dir13
    lr = config.lr
