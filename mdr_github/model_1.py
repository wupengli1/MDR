        ...
        for hisbatch in range(len(text_his)):
            ...
            if text_his_len == 0:
                ...
                res2 = self.user_memnet1(
                    ...
                    profile=profile_res)
                # if is_train:
                sorce_cha1 = (res2 - x_start_response_gpt) ** 2
                # a = cos_sim(res2, x_start_response_gpt)

                loss_yuyi1 = sorce_cha1.mean(dim=list(range(1, len(sorce_cha1.shape))))#/768
                if hisbatch == 0:
                    loss_response =  loss_yuyi1
                else:
                    loss_response += loss_yuyi1
            else:
            ...
            if not is_train:
                if lastre:
                    x_t_t = res2
                    x_t_t1 = x_start_response_gpt
                    # loss_a = a
                    lastre = False
                else:
                    x_t_t = torch.concat((x_t_t, res2))
                    x_t_t1 = torch.concat((x_t_t1, x_start_response_gpt))
                    # loss_a += a
