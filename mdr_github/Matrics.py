...
class NLP_Gen_Matrics:
    def __init__(self,condition_list=['test'],model_name='test_model',resnumber='0'):
    ...
    def get_result(self,persona_list,context_list,pred_list,target_list):
        ...
        # Save the result
        with open('./result' + self.resnumber+'/'+ file_name + '-metrics.txt','w',encoding='utf-8') as f:
    ...
    def get_consistency(self,persona_list,context_list,pred_list,target_list):
        ...
        if self.config.model_metric == True:
            ...
            self.judge_value_list['consistency-coherence'] = y_hat.count(0) / len(y_hat)
            self.judge_value_list['consistency'] = (y_hat.count(0) + y_hat.count(2)) / len(y_hat)
