...
class Net(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        if self.data_language == 'EN':
            ...
            with open("test.txt", encoding='utf-8') as f:#os.getcwd() +  self.data_path[0] +
                data_single = f.read()
            ...
            eos_token = '<|endoftext|>'
            features_train = torch.load('train.pt')
            features_val = torch.load('val.pt')
            features_test = torch.load('test_sample.pt')
            features_train_t = features_train[0]
            for l in range(1,len(features_train)):
                features_train_t = torch.cat((features_train_t,features_train[l]),0)
            features_val_t = features_val[0]
            for l in range(1,len(features_val)):
                features_val_t = torch.cat((features_val_t,features_val[l]),0)
            features_test_t = features_test[0]
            for l in range(1,len(features_test)):
                features_test_t = torch.cat((features_test_t,features_test[l]),0)

            features_train_true = torch.load('train_true.pt')
            features_val_true = torch.load('val_true.pt')
            features_test_true = torch.load('test_true.pt')
            features_train_true_t = features_train_true[0]
            for l in range(1, len(features_train_true)):
                features_train_true_t = torch.cat((features_train_true_t, features_train_true[l]), 0)
            features_val_true_t = features_val_true[0]
            for l in range(1, len(features_val_true)):
                features_val_true_t = torch.cat((features_val_true_t, features_val_true[l]), 0)
            features_test_true_t = features_test_true[0]
            for l in range(1, len(features_test_true)):
                features_test_true_t = torch.cat((features_test_true_t, features_test_true[l]), 0)
        else:
        ...
        for line in data_single_train:
            ...
            data_train.append(temp + [temp[1] + temp[2]] + [history_t]+[features_train_t[line_i]]+[features_train_true_t[line_i]])
            line_i += 1
        ...
        for line in data_single_test:
            ...
            data_test.append(temp + [temp[1] + temp[2]] + [history_t] + [features_test_t[line_i]]+ [features_test_true_t[line_i]])
            line_i+=1
        ...
        for line in data_single_val:
            ...
            if temp[2][-1] not in '!.?)"\'':
                temp[2] += '.'
            temp[2] = temp[2] + eos_token
            data_val.append(temp + [temp[1] + temp[2]] + [history_t]+ [features_val_t[line_i]] + [features_val_true_t[line_i]])
            line_i +=1
    ...
    def collate_fn_text(self, batch):
        ...
        inputs = {'persons': [], 'posts': [], 'responses': [], 'p_r': [], 'his': [],'features':[],'features_true':[]}#,'features_true':[]
        for line in batch:
            ...
            inputs['features'].append(line[5])
            inputs['features_true'].append(line[6])
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        with open(os.getcwd() + '/result' + resnumber + '/' + self.model_file + '-pred_result.txt', 'w',
                  encoding='utf-8') as f:
            f.write('\n\n'.join(pred_list))
...
if __name__ == "__main__":
    ...
    output_path = config.output_dir2
    lr = config.lr
    ...
    best_model_path = './Model_26/' +  os.listdir('./Model_26')[-1]
    d = torch.load(best_model_path)["state_dict"]
    net.load_state_dict(d, strict=False)
