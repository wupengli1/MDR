...
class Decoder(pl.LightningModule):
    ...
    def __init__(self):  # dropout
        ...
        try:
            ...
            if data_language == 'EN':
                self.tok = GPT2Tokenizer.from_pretrained("/media/wupengli/premodel/premodel/gpt2", do_lower_case=False)
                self.model = GPT2LMHeadModel.from_pretrained("/media/wupengli/premodel/premodel/gpt2",
                                                             )
                self.tok.pad_token = '<pad>'
            else:
                self.tok = BertTokenizer.from_pretrained(
                    "/media/wupengli/premodel/premodel/gpt2chinesecluecorpussmall")  # uer/gpt2-chinese-cluecorpussmall
                self.model = GPT2LMHeadModel.from_pretrained(
                    "/media/wupengli/premodel/premodel/gpt2chinesecluecorpussmall",
                    ignore_mismatched_sizes=True)
        self.config = config
    ...
    def forward(self, if_train, p, x, y, x_y, z_r, z_p, sim_score):  # The initial state [layers*directions, batch, dim]
        ...
        if if_train:
            ...
            outputs = self.model(input_ids=batch_input_encode_x_y['input_ids'],
                                 ...
                                 labels=batch_input_encode_x_y['labels'], return_dict=True)
